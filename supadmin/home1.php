...
<body class="bg-gray-50 min-h-screen">
        ...
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2 text-sm text-gray-500">
                    ...
                    <span id="pending-count"><?php echo $pending_count; ?> Pending applications</span>
                </div>
            </div>
            ...
            <div class="grid gap-6" id="user-cards-container">
                <?php
                $stmt = $conn->prepare("SELECT * FROM tutors WHERE status = 'Pending'");
                $stmt->execute();
                ...
                if (count($result) > 0) {
                    foreach ($result as $row) {
                        ...
                        echo "<form action='' method='post' onsubmit='return confirmAction(this, \"approve\")' class='inline'>";
                        ...
                        echo "<form action='' method='post' onsubmit='return confirmAction(this, \"reject\")' class='inline'>";
                        echo "<input type='hidden' name='id' value='{$row['id']}'>";
                        echo "<button type='submit' name='reject' class='inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-danger rounded-lg hover:bg-red-600 transition-colors duration-200'>";
                        echo "<svg class='w-4 h-4 mr-2' fill='none' stroke='currentColor' viewBox='0 0 24 24'>";
                    ...
                    echo "</svg>";
                    echo "<h3 class='mt-2 text-sm font-medium text-gray-900'>No pending tutors</h3>";
                    echo "<p class='mt-1 text-sm text-gray-500'>No tutors are pending approval at this time.</p>";
                    echo "</div>";
                }
                ?>
            </div>
    ...
    <script>
        function confirmAction(form, action) {
            ...
            return confirm(`Are you sure you want to ${actionText} this tutor?`);
        }
    ...
    <!-- Reject Button -->
    <form action="" method="post" onsubmit="return confirmAction('reject', '${tutor.name}')" class="inline">
        <input type="hidden" name="id" value="${tutor.id}">
        <button type="submit" name="reject" class="inline-flex items-center px-6 py-3 text-sm font-medium text-white bg-red-600 rounded-lg hover:bg-red-700 transition-colors duration-200">
            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
            </svg>
            Reject
...
function confirmAction(action, tutorName) {
    const actionText = action === 'approve' ? 'approve' : 'reject';
    const message = `Are you sure you want to ${actionText} ${tutorName}?`;
